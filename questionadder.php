<?php
        ...
        if (isset($_POST['content'])&&isset($_POST['title'])) {
            ...
            if (isset($received[0])) {
                $additionalinfo="<script>alert('Your submission was not posted because it is a duplicate of an already posted question);</script>";
            } else {
                ...
                if (null!=$_POST['email']&&strlen($_POST['email'])>4) {
                    $query.=','.verify($_POST['email']);
                } else {
                    $query.=",NULL";
                }
                $query.=')';
                ...
                if (isset($received[0])) {
                    ...
                    if (null!=verify($_POST['title'], 'n')) {
                        $pageinfo['content'].='title is '.verify($_POST['title'], 'n');
                    }
                    if (null!=verify($_POST['content'], 'n')) {
                        $pageinfo['content'].=' and content is '.verify($_POST['content'], 'n');
                    }
                    if (null!=verify($_POST['email'], 'n')) {
                        $pageinfo['content'].=' and emails is '.verify($_POST['email'], 'n');
                    }
            ...
            if (null!=verify($_POST['title'], 'n')) {
                $pageinfo['content'].=verify($_POST['title'], 'n');
            }
            if (null!=verify($_POST['content'], 'n')) {
                $pageinfo['content'].=verify($_POST['content'], 'n');
            }
            $pageinfo['content'].=')">link</a> so I can work to fix this immediately.';
        }
